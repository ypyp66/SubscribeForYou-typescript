...
class SubscribeDatas(models.Model):
    ...
    subs_pk = models.ForeignKey('SubscribeListData', db_column='subs_pk', related_name='subs', verbose_name='구독정보', on_delete=models.CASCADE)
...
class SubscribeListData(models.Model):
    ...
    def __str__(self):
        return '%d [%s] %s' % (self.pk, self.category, self.title)